...
import com.taskdesignsinc.android.myanimeviewer.adapter.HistoryRecyclerAdapter;
...
public class HistoryMaterialFragment extends Fragment
    ...
    private static final String TAG = HistoryMaterialFragment.class.getSimpleName();
    ...
    private ArrayList<HistoryRecord> mList;

    private HistoryRecyclerAdapter.OnItemClickListener mClickListener = new HistoryRecyclerAdapter.OnItemClickListener() {
        @Override
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        if (activity != null) {
            mAdapter = new HistoryRecyclerAdapter(mList, getActivity(), mClickListener);
    ...
    protected void deleteHistory(int index) {
        MAVApplication.getInstance().getRepository().deleteHistoryRecord(mAdapter.getItem(index).getEpisodeUrl());
    ...
    public void refresh() {
        ...
        if (activity != null) {
            if (mHasChanged) {
                if (mAdapter != null)
                    mAdapter.clearData();
                WriteLog.appendLog(TAG, "Refreshing Online History");
    ...
    @Override
    public void onLoadFinished(Loader<List<HistoryRecord>> arg0,
                               List<HistoryRecord> data) {
        switch (mSortType) {
            case 0:
                Collections.sort(data, HistoryRecord.SortByTimeStampDESC);
                break;
            case 1:
                Collections.sort(data, HistoryRecord.SortByTimeStampASC);
                break;
        }
        // Set the new data in the adapter.
        mAdapter.addHistoryRecordList(data);
        mAdapter.getFilter().filter(null);

    }

    @Override
    public void onLoaderReset(Loader<List<HistoryRecord>> arg0) {
